...
//		}
	...
	public static boolean setItemsTo(Player player, ItemStack to) {
		...
		if ("ghost_visibility_toggle".equals(ItemRegistry.getScarceTypeByItem(player.getItemOnCursor()))) {
			player.setItemOnCursor(to);
		}
		
		if ("ghost_visibility_toggle".equals(ItemRegistry.getScarceTypeByItem(player.getInventory().getItemInMainHand()))) {
			player.getInventory().setItemInMainHand(to);
		} else if ("ghost_visibility_toggle".equals(ItemRegistry.getScarceTypeByItem(player.getInventory().getItemInOffHand()))) {
			player.getInventory().setItemInOffHand(to);
		}
		
		ItemStack[] inv = player.getInventory().getContents();
		for (int i = 0; i < inv.length; i++) {
			if (inv[i] != null && "ghost_visibility_toggle".equals(ItemRegistry.getScarceTypeByItem(inv[i]))) {
